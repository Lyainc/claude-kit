...
# but content drifted) is exit 1. Case 9 (#659) cross-checks install-hooks.sh's awk
...
# Expected: OK: all 44 check-skill-reference-drift self-test cases passed
...
# Expected: OK: all 40 check-skill-token-budget self-test cases passed
...
# distill at all. Two prose skills, so this is a static-content check like
...
# reverts), and distill's DROP list must carry the recurrence floor + default-behavior + a bounded
# already-landed grep.
...
# Expected: OK: all 11 distill-gate-routing checks passed.
