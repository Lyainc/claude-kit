...
6. **Update project back-references** (only when Step 3 produced project selections):
   ...
   - **YAML safety**: if `{topic}` contains any character outside `[a-z0-9-]` (the kebab-case filename invariant from Step 4), abort Step 6 for that project, log a warning naming the unsafe topic, and add the project to the `skipped_projects` list reported in Step 7. The note file itself is still created.
