...
- `vault-bridge/`: Obsidian vault I/O bridge with the `vault-searcher` agent, slash-command style workflows, hook scripts, session-note support, manifest caching, vault write governance, and plan-doc sync.
...
| Session record | Not owned here | `vault-searcher` Mode 4 session-note creation |
