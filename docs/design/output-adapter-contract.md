...
이 문서 = **논리 계약**(어댑터 호출 규약·매핑·체이닝). ②를 단일 플러그인으로 묶을지(graphify·OVM·spec-first가 이미 각자 플러그인) 분산 유지할지의 **물리 구조는 #102(G3 wave)** 결정이에요. 어댑터 계약은 구조 독립적이라 #102 어느 결과에도 불변 — #101 Acceptance는 #102 없이 충족(G2 쟁점 표 "출력 레이어 물리 구조" 행 정합).
