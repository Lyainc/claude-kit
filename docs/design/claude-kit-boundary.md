...
> **③↔④ 경계 판정 기준 + `/wiki` 배포 단위 (2026-07-02 #304 → 2026-08-20 #645 재판정. 이 블록은
> 07-02 straddler 블록과 08-18 재확인 블록 2개를 대체한 것으로, 덧붙인 게 아니에요 — §5 "선례 편철"
> 참조)**: 레이어 번호는 "누가 뭘 하나"를 안 알려줘서, 사서/브리지 한 줄 테스트를 판정 기준으로
> 채택했어요 — **"이게 프로젝트가 존재한다는 걸 알아야 하나?"** 몰라도 됨 → **사서(④ OVM)**, 볼트
> 코퍼스만 다뤄요. 한쪽에 프로젝트가 없으면 무의미 → **브리지(③ vault-bridge)**, 프로젝트↔볼트 양쪽
> 레퍼런스를 동시에 쥔 유일한 컴포넌트예요. wiki의 U7 route(다른 레포에서도 참인가 라우팅)는 크로싱
> 판단=③ 브리지 로직, compile/dedup=④ 사서 코어인 **합법적 straddler**로 명시해요 — 이 straddler
> 분류 자체는 2026-08-20 재판정에서도 유지됩니다. 그리고 A-only 재설계 이후 브리지 정체성은
> **pull-mostly**로 바뀌어요 — repo→vault로 문서를 복사해 나르는 ferry(원본이 바뀌면 drift하는 stale
> 원인, G21/G24/G26으로 이미 대부분 철거)는 금지, repo-세션에서 사실을 우려내는 wiki compile(mirror가
> 없어 구조상 drift 불가)은 유지, A↔B(wiki↔second brain) 연동은 링크only(복사 금지)예요. 근거: #304
> (판정 근거였던 `vault-role-redefinition-draft.md`는 승격 완료 후 삭제 — 이 문서가 SSOT).
>
> **`/wiki` 배포 단위(어느 플러그인에 실려야 호출 가능한가)는 위 straddler 판정과 별개 쟁점입니다.**
> #304는 "wiki의 U7 route" 하나의 크로싱/코어 분류만 판정했고("합법적 straddler"), 배포 단위는 판정
> 범위 밖이었다는 점을 그 조문 자체가 명시했어요(스코프 유보 문구, 결과에 배포처를 정하는 항 없음).
> 08-18 재확인은 #304를 배포 단위 판정으로 오적용해 이관을 기각했는데, 근거 3가지 중 2가지가
> 08-20 격리 expert-panel 재검증(7인 독립 진술→반박 교환 2라운드, e1 이관 6:1 → e2 7:0, 유일한
> 반대가 e2에서 스스로 전환)에서 반증됐습니다 — "OVM은 프로젝트에 못 깔린다"는 실측 전제는 반증됐지만
> owner 확인 결과 그 설치(PhototicketMaker, 2026-07-29·30)는 의도한 동작이 아니라 #477(2026-08-02)에
> 이미 기록된 플러그인 MECE 위반이었고, "이관하면 OVM 단독 설치가 vault-bridge 의존이 된다"는 근거는
> 설정 전수 census 결과 OVM 단독 설치가 이 머신에 0곳이며 이미 audit가 `.vault-bridge/manifest.json`을
> 읽어 의존이 기존재한다는 사실로 반증됐습니다(이관은 그 의존을 2건→1건으로 줄임). 전문·수집 조건·
> 미해결 항목: [이슈 #645](https://github.com/Lyainc/claude-kit/issues/645) 2026-08-20 코멘트.
>
> **확정 (2026-08-20, #645)**: `skills/wiki/` · `scripts/manifest-wiki-match.py` ·
> `reference/obsidian-format.md`는 **vault-bridge**로 이관. `test-manifest-reads.py`는 9(OVM 잔류,
> 이름 유지)/4(vault-bridge 신규 `test-manifest-wiki-match.py`)로 분할. **audit E12 전체(wiki
> self-audit)는 OVM 잔류** — `verified:`/`provenance:`를 쓰는 쪽(vault-bridge `/wiki`)과 읽는 쪽(OVM
> `audit` E12)이 이제 플러그인 경계를 사이에 두므로, 그 필드명 계약은 크로스플러그인 pin 테스트로
> 고정합니다(현재 0건 → 신설). `/base`·`/audit`·에이전트 2개는 OVM 잔류. 동반 조건(볼트 부재 시
> 중단·고지, exit-3 사용자 경고, 메이저 범프 + forwarding shim 금지)은 이슈 #645 §4 참조.
>
...
> **선례 편철 (2026-08-20, #645)**: ① 이 문서군의 규칙을 개정·재확인하는 판정은 **개정 대상 문서의
> `최종 개정` 헤더와 개정 대상 절 제목에 인용된 이슈 번호**를 선례 집합으로 삼아 그 본문·코멘트를
> 읽고, 동일 쟁점을 심리한 것이 있으면 판정문에서 인용해 구별하거나 파기한다. ② 그 집합 **밖의
> 이슈를 찾아 나설 의무는 없다.** ③ 대신 어떤 이슈에서 이 문서군의 규칙을 뒤집거나 제약하는 결론이
> 나오면, **해당 절 머리말에 그 이슈 번호를 한 줄 추가하는 것이 그 이슈의 종결 요건**이다. 편철되지
> 않은 규범은 후속 판정에 대항하지 못한다. ③이 유실을 막고 ②가 무한 탐색을 막는다. 판정 근거 전문은
> scratchpad가 아니라 GitHub 이슈 코멘트에 인라인으로 남긴다 — 08-18 재확인이 파기된 계기 중 하나가
> "사라진 scratchpad 경로를 가리켜 이유 전문이 GitHub에 안 남는" 결함이었다(#645).
