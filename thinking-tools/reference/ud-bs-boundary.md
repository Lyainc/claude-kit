...
공격) / doc-concretize(자유 서술 문서화)는 이 축 바깥이다 — 넷 다 "답의 모양"이 아니라 "인터뷰
자체를 하는가"에서 이미 갈린다. doc-concretize는 재귀적 글쓰기이지 Y/N 체크리스트 인터뷰가
아니다. 다만 "구체화"라는 단어 하나가 build-spec 쪽과 겹쳐 보이는 문제는 실제로 있었다 — 아래
"구체화" 단어의 귀속 참조 (#431).
...
두 스킬 모두 같은 타이밍 정책을 쓴다 — **채점은 기본 인라인, 격리 Agent 재채점은 게이트가
임박한 라운드에서만**. 매 라운드 서브에이전트를 부르는 건 바뀌지도 않을 결과에 매번 비용을
쓰는 것이고, 그렇다고 매번 인라인만 쓰면 인터뷰어가 자기 인터뷰를 자기 채점하는 자기검증
편향이 그대로 남는다(#433). "임박"의 구체적 정의는 두 스킬의 게이트 조건이 서로 다르므로
각자의 reference에 있다 — 같은 것은 정의 위치가 아니라 **정책 자체(기본 인라인 + 임박 시에만
격리)**다:
...
**재드리프트 방지**: 이 일치를 지키는 자동 CI 검증은 없다 — 두 스킬 중 하나의 타이밍 정책을
고칠 때 이 섹션과 다른 스킬의 reference를 나란히 대조하는 건 리뷰어의 수동 규율이다. 스크립트로
못 잡는 이유는 아래 "왜 이 표를 CI로 자동화하지 않는가"와 같다: 실제로 어길 수 있는 지점은
텍스트 문자열이 아니라 "언제 Agent를 부르는가"라는 런타임 판단이다.
...
| "이 아이디어 구체화해줘" | doc-concretize | "구체화"만 단독 — 산출물 키워드(스펙/명세/seed)가 없으면 자유 서술 문서가 기본값 |
| "이 아이디어를 스펙으로 구체화해줘" | BS | "스펙으로"가 명세 산출물을 직접 지목 — bare "구체화"와는 다른 축 |

## "구체화" 단어의 귀속 (#431)

`unknown-discovery`↔`build-spec` 두 스킬 사이의 축과 별개로, "구체화"라는 단어 자체는
doc-concretize의 것이다 — 단독으로 쓰이면(스펙/명세/seed 같은 산출물 키워드 없이) 자유 서술
문서가 기본값이다. build-spec 쪽이 실제로 소유하는 트리거는 `build-spec/SKILL.md`에 그대로
있는 "아이디어를 스펙으로"이지, "아이디어 구체화"라는 별도 축약어가 아니다 — 축약어를 쓰면서
글자 "구체화"가 겹쳐 보였을 뿐, SKILL.md description 레벨에서는 애초에 겹친 적이 없다.
