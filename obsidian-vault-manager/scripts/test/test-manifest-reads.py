#!/usr/bin/env python3
"""manifest-summary.py regression (#468, mirrors #460's
feedback-loop/scripts/test/test-e8-candidates.py).

The script exists so audit/SKILL.md never `cat`s .vault-bridge/manifest.json directly — the
harness truncates large Bash output to a 2 KB preview before the model sees it, so a raw `cat`
...
statically greps the live audit/SKILL.md call site to pin that it never regresses back to a raw
`cat` of the manifest.

**Scope note (#645).** This file used to cover `manifest-wiki-match.py` and `wiki/SKILL.md` too.
`/wiki`'s deployment unit moved OVM -> vault-bridge, and the test followed its script: that half
now lives in `vault-bridge/scripts/test/test-manifest-wiki-match.py` under a DIFFERENT name,
because the two files pin two different contracts and must stay distinguishable in a grep or a
CI log. The two halves shared only three helpers and the string "2 KB", so nothing here depended
on what left.
