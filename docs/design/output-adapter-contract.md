...
| # | 한계 | 어디서 |
|---|------|--------|
| L1 | ~~**닫힌 이슈 미조회**~~ — **해소됨 (#489)**: Phase 0 백로그 스캔이 이제 `--state open`과 `--state closed` 양쪽을 다 훑어요(`backlog-prefilter.py`의 `build()`가 `gh issue list --state open` + `--state closed`를 둘 다 호출), COMPLETED로 닫힌 결정(= "이미 정해졌으니 반대로 가지 마라")도 후보로 올라와요. |
...
| L3 | ~~**bug 템플릿 형식 도달 불가**~~ — **해소됨 (#502, 이후 일반화)**: 고정 필드 매핑 대신 `issue-raise`가 템플릿 헤딩을 호출 시점에 그대로 따라요 — 처음엔 `.github/ISSUE_TEMPLATE/*.md`를 직접 `Read`했지만, 지금은 `thinking-tools/scripts/issue-template.py`가 GitHub 기본 스캐폴드 파일명·`.yml` 이슈 폼·템플릿 없음까지 다 처리하고 그 결과를 같은 방식으로 넘겨요. 템플릿이 바뀌면 출력도 같이 바뀌고 스킬 안에 헤딩을 하드코딩하지 않는다는 원래 원칙은 그대로예요. |
