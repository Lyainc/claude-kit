...
# skills/retro/SKILL.md (Phase 1 stamp, Phase 2 emit). Inlining it in two
...
#       Phase 1 and Phase 2 are separate Bash-tool calls (fresh shell each
...
#       argument to Phase 2's `emit` call below.
#   retro-telemetry.sh emit <start_ms> <processed> <deduped>
#       Append ONE schema-valid retro skill_invoke line whose meta carries the
#       two retro counters PLUS duration_ms (now − start_ms; null when
#       start_ms is missing/non-numeric). (items_promoted was dropped with
#       the PROMOTE phase, #480 — retro no longer writes the vault. budget_used
#       was dropped with the budget/priority machinery, #639 — retro no longer
#       caps or prioritizes what it processes.)
...
case "${1:-}" in
  ...
  emit)
    ...
    START_MS="${2:-}"; PROCESSED="${3:-0}"; DEDUPED="${4:-0}"
    END_MS="$(now_ms)"
    ...
    LINE="$(jq -nc \
      ...
      '{ts:$ts, session_id:$sid, cwd:$cwd, plugin:"feedback-loop",
        ...
        meta:{retro_items_processed:$processed,
              items_deduped:$deduped, duration_ms:$duration}}' 2>/dev/null)"
