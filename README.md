...
| [obsidian-vault-manager](obsidian-vault-manager/) | `0.10.0` | 스킬 7 + 에이전트 2 + scripts (ovm-primitives) |
| [vault-bridge](vault-bridge/) | `1.7.1` | 에이전트 1 + 훅 6 (Stop / SessionEnd command+prompt / SessionStart / PreToolUse Read\|Grep\|Glob / PreToolUse Write\|Edit) + 슬래시 커맨드 5 (`/save-session`, `/vault-link`, `/vault-manifest-refresh`, `/vault-commit`, `/save-plan-doc`) (구 `vault-reader`) |
