...
|----|------------------|------|----------|------|
| **N1** | dynamic Workflow | `agent()`/`parallel()`/`pipeline()`/`phase()`/`log()`, schema 구조화 출력, worktree 격리, budget, resume, nested workflow | 동시 에이전트 cap = `min(16, cores−2)`; 총 1000 에이전트; **중첩 1단계**(workflow 안 workflow 1회); `Date.now`/`Math.random`/argless `new Date()` 사용 불가; **resume 동일 세션 한정**. **[CONFIRMED 2026-06-10]** PR #205 dogfood(커밋 99a3f7a·57961c6·48417e9·33146e6) 실측 4종: ① **meta-first**: `export const meta`가 스크립트 최상단 첫 문장이어야 함 — 아니면 스크립트 자체가 거부됨. ② **top-level body 실행 모델**: `export default` 함수는 *호출되지 않음* — body가 async context로 직접 실행되며 `agent()`/`phase()`/`args`가 전역 주입됨. ③ **args 문자열화**: 호출 표면에 따라 args가 JSON 문자열로 도착할 수 있음 → 스크립트가 parse-normalize 해야 함. ④ **agentType 한정자 필수**: 에이전트 레지스트리는 정규화된 이름(예: `thinking-tools:thinking-facilitator`)만 해석 — 비한정 `thinking-facilitator`는 미해석(오류 메시지에 레지스트리 목록 포함); N3에도 해당 | `[SPEC]` (Workflow 도구 정의 — 메인 세션 도구 명세 기재값) · 공개 corroboration(#200): [dynamic-workflows 블로그](https://claude.com/blog/a-harness-for-every-task-dynamic-workflows-in-claude-code) — 프리미티브·`~/.claude/workflows` 공유 표면·비용 경고("not needed for every task … significantly more tokens")는 공개 1차 출처로 확인, 단 위 *한계 수치*까지는 블로그가 안 다뤄 티어는 `[SPEC]` 유지(도구 정의가 여전히 1차) · `[CONFIRMED 2026-06-10]` PR #205 dogfood 실측(커밋 99a3f7a·57961c6·48417e9·33146e6) |
...
| **N3** | Agent / Task 도구 | 서브에이전트 spawn, `run_in_background`, `SendMessage`로 컨텍스트 이어 재호출, `agentType`(커스텀 에이전트 레지스트리), worktree 격리 | 서브에이전트 **최종 메시지는 호출자에게만** 반환(사용자 비노출) → **중간 critique 과정 관찰 불가**(INV-2 enforce 설계 제약, §4.2 참조); 새 Agent 호출은 fresh context(SendMessage만 컨텍스트 유지); **[CONFIRMED 2026-06-10]** `agentType`은 **한정자 필수** — 비한정 이름(예: `executor`)은 레지스트리 미해석(N1 ④와 동일 제약, 커밋 33146e6) | `[SPEC]` (Agent 도구 정의) · `[CONFIRMED 2026-06-10]` PR #205 dogfood(커밋 33146e6) |
