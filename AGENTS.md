...
## Project guidance migrated from root `CLAUDE.md`

This repository is `claude-kit`, a Claude Code plugin marketplace that is also operated from Codex/OMX. Root `CLAUDE.md` remains the Claude Code contributor entrypoint; this section is the active Codex/OMX migration of its project-specific guidance.
...
- `feedback-loop/`: layer ⑤ self-improvement plugin (measure→review→keep — NOT an execution engine; **externally distributed**, #217). Skill: `retro` (audit E8 promotion + 3-branch output + dedup + budget) + absorbed telemetry (opt-in `CLAUDE_KIT_TELEMETRY=1`, deterministic event-logger hooks, zero per-turn LLM cost, local-only). One-way dependency (CON-5): reads leaf OUTPUT + telemetry events, never imports leaf code.
...
- Runtime behavior rules for installed plugins belong in each plugin's agent/skill descriptions and bodies; do not duplicate long runtime behavior in root guidance.
- Skill and agent instruction bodies should be English for LLM parsing.
- Metadata keys and section headers should be English.
- User-facing output from plugin skills/agents should be Korean when the file specifies the Korean I/O directive.
- README descriptions default to English; Korean trigger examples are acceptable.
- Reference docs and examples under `reference/` or `examples.md` may be Korean.
- Preserve Korean text in templates/examples that represent actual vault content.

### Marketplace layout

- `.claude-plugin/marketplace.json`: marketplace manifest; each plugin entry's `source` points at the plugin root.
...
| `note` | `{topic}.md` | `30_Notes/` |
| `project` | `_index.md` | `20_Projects/{name}/` |

Same-date collisions append `-v2`, `-v3`, and so on. Standard frontmatter is `created`, `tags`, `type`, plus conditional `status` for session handoffs or projects.

### Project validation

Run these checks after marketplace/plugin changes:
...
# feedback-loop telemetry schema + report regression (#217 — telemetry absorbed)
