...
All three are AskUserQuestion prompts. SKILL.md's Invocation Detection picks which to fire;
the bodies live here. Each ends with the standard `일반 응답 (Standard response)` opt-out.

**Mode A — implicit Explore trigger** ("다양한 아이디어", "브레인스토밍", "여러 대안", "alternatives"):
```
...
**Mode Disambiguation** — ambiguous input like "다양하게 써줘"; the pick resolves the mode AND
doubles as confirmation, so Phase 0 step 4 does not prompt again:
```
어떤 작업이 맞을까요?
