...
rm -rf "$TMPD" 2>/dev/null || true   # P4: a mktemp fixture is a real permanent delete, so -rf is the honest form (CI: ephemeral runner cleans up)
...
rm -rf "$TMPD2" 2>/dev/null || true   # P4: same — permanent delete of a mktemp fixture is the intent
