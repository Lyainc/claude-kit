...
* **obsidian-vault-manager, vault-bridge:** `/wiki`'s deployment unit moves from obsidian-vault-manager to vault-bridge ([#645](https://github.com/Lyainc/claude-kit/issues/645)). A 2026-08-18 re-confirmation had kept `/wiki` in obsidian-vault-manager, citing #304 as having already settled plugin ownership; a 2026-08-20 isolated expert-panel re-verdict (7 independent statements → 2 rounds of rebuttal, e1 6:1 → e2 7:0, the lone holdout switching sides in e2) overturned it — #304 only ever classified the U7-route *crossing* logic as a legal straddler between ③/④, never the deployment unit, and two of the 08-18 verdict's three grounds (an owner-unintended OVM install cited as evidence the "OVM can't live in a project" premise was false; an independent-installability regression) did not survive re-examination. Moved: `skills/wiki/`, `scripts/manifest-wiki-match.py`, `reference/obsidian-format.md`. `scripts/test/test-manifest-reads.py` split along its two independent contracts (9 audit/SKILL.md pins stay in obsidian-vault-manager under the same name; the 4 wiki/SKILL.md pins moved to vault-bridge's new `test-manifest-wiki-match.py`). `audit` (E1–E3, E5–E6, E9–E12 incl. E12 wiki self-audit), `base`, and both agents stay in obsidian-vault-manager. Four conditions landed with the move: (1) both `/wiki` and `/vault-save` now stop and tell the user instead of `mkdir -p`-ing a vault root into existence when none is configured — a vault created that way could never receive a manifest, since the manifest hook no-ops on the same missing-root check, degrading recall/dedup silently forever (this also closes [#697](https://github.com/Lyainc/claude-kit/issues/697)); (2) a manifest-read failure during wiki DEDUP now surfaces a one-line warning that matching degraded to filename/slug-only; (3) a new cross-plugin pin (`vault-bridge/scripts/test/test-wiki-frontmatter-contract.py`) locks the `verified:`/`provenance:` field names shared between the writer (vault-bridge `/wiki`) and the reader (obsidian-vault-manager `audit` E12), which previously lived in one plugin and shared a test run — a field rename on either side used to silently pass with degraded output instead of failing; (4) obsidian-vault-manager's README carries a redirect line — no forwarding shim, since one active in both plugins at once would make routing non-deterministic. Full verdict, evidence, and self-corrections: issue #645's 2026-08-20 comment. `docs/design/claude-kit-boundary.md` §2 (single source for the deployment-unit rule) and `docs/design/vault-second-brain-v5.md` §9 (now a one-line pointer to it) were updated together so the rule stops being defined in two places.

### Enhanced

* **thinking-tools:** shared persona pool closes improvement-matrix W8 · M1 · W4 ([#418](https://github.com/Lyainc/claude-kit/issues/418)). `expert-panel` and `adversarial-review` each invented personas inline per session, so re-reviewing the same decision produced a different panel and the two skills could name the same viewpoint differently — two judgements that cannot be laid side by side. **W8 · M1 (페르소나 라이브러리)**: `thinking-tools/reference/personas.md` holds 10 domain experts, each with a distinct evaluation criterion (the axis `expert-panel` already required to keep roles from collapsing into one opinion) and a tag list. Selection is **deterministic tag matching**, not per-call LLM choice — score each entry by distinct tags occurring in the lowercased topic text, rank by hits desc then ID asc, and cut: ≥3 matches take the top `min(5, matched)`, 1–2 matches fill up to the 3-expert floor with ad-hoc personas, 0 matches go fully ad-hoc. Free choice would have made "run it twice, get the same set" a coin flip rather than a check. Latin tags match only at a **word start**, Hangul tags as plain substrings (Korean compounds have no word boundaries) — raw substring matching scored `ui` against "b**ui**ld", `db` against "feed**b**ack" and `doc` against "**doc**ker", and since selection is deterministic *and* silent, a false hit outranks a real one with nobody looking. `thinking-tools/scripts/test/test-persona-selection.py` executes the rule against the live tag table and guards exactly that, so a future tag edit cannot reintroduce it. The pool stays a **default, not a closed list**: off-pool topics proceed with `{Domain} Expert (ad-hoc)` personas recorded as `adhoc:{n}` in the STATE block, so the fallback is visible instead of silent. Only **domain experts** are shared — `adversarial-review`'s Attacker / Judge / Steelman Coach are topic-independent *role* labels and stay out of the pool; its contact point is the rank-1 entry supplying the Attacker's *domain angle* (recorded as STATE `Angle`). Both skills run the rule on the **user's original topic text** and every cut starts at rank 1, so they land on one entry (the shared input is what carries that — see #423, which closed the gap where `adversarial-review` selected from the model-authored Steelman instead). M1's companion note ("temperature variation") is explicitly out of scope — the pool has to exist before there is anything to vary. **W4 (출력 조합성)**: `adversarial-review` cited the common output schema nowhere, leaving its exported report unparseable prose; the Final Report Template now emits the common frontmatter block plus `claims_tested` / `verdicts` / `angle`, and `angle` is registered in `reference/common-schema.md` as an additive extension field.
...
* **thinking-tools:** the persona Selection Rule now actually controls both skills' execution ([#423](https://github.com/Lyainc/claude-kit/issues/423)). #418's determinism held on paper and passed acceptance, but on three points nothing enforced it. (1) **The rule ran on two different inputs**: `expert-panel` used the topic text while `adversarial-review` used the *finalized Steelman* — model-authored text, so two runs of one claim produce two Steelmans and can select two different entries. Measured 2026-07-22: one claim scored `[P1 P2 P6]` as submitted and `[P1 P2 P3 P7]` after Steelman construction; rank 1 matched only by coincidence. The input is now the **user's original topic text** in both skills — chosen over merely narrowing the claim in `personas.md`, because narrowing would have left `adversarial-review` non-reproducible against *itself*, and because a compacted session may no longer hold the Steelman verbatim while the submitted claim is always recoverable. (2) **The guard was a tautology**: `test-persona-selection.py`'s check 5 asked whether `ids[0]` was inside `ids[:min(5, len(ids))]`, which is true for any non-empty selection on any pool — it could not fail. It now pins the measured divergence pair as a fixture and asserts the doc contract (each consuming file names the shared input; none aims the rule at the Steelman); run against the pre-fix files it reports 4 failures. (3) **A free-judgment clause sat next to the deterministic rule**: `expert-panel`'s "if every selected entry is implementation-focused, add a strategy-focused ad-hoc persona" is an LLM call that would emit different `adhoc:{n}` for one topic across runs — removed, since strategy vocabulary already matches `P9` through the tags.
