...
- **feature-full**:
  ...
  3. **On explicit confirmation only**: invoke the workflow script —
     ...
     `code-reviewer`). These were OMC-provided and removed with OMC (#204); native
     rewiring is tracked in #268. If the environment's agent registry only resolves
     **qualified** names, pass them via the `impl_agent_type` / `critique_agent_type`
     args overrides — the script's CON-3 disjoint assert applies to the resolved pair
     either way, and an unresolvable agentType fails fast with the registry listing
     (0 tokens spent).
