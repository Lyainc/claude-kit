...
| `workflow-harness/skills/*` (`retro`, `handoff-plan`, `slice-router`) + `workflow-harness/.claude-plugin/plugin.json` | Layer ⑤ execution harness on native primitives — goal-doc routing/validation (INV-4), E8 retro promotion, backlog→goal-doc handoff | Codex drives the same goal-doc/slice loop over native `/goal` + Workflow + role routing; harness skills are exposed as Codex plugin skills or documented fallbacks. The harness orchestrates native primitives, so parity is behavioral (same slice routing + invariant gating), not a literal skill-name copy | Active equivalent for installed plugin | `find workflow-harness/skills -name SKILL.md`; `python3 -m json.tool workflow-harness/.claude-plugin/plugin.json`; `python3 workflow-harness/scripts/test/test-router.py` + `test-invariant.py` |
