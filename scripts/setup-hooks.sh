...
FEATURES:
  ...
  ✓ Version bump warning (not blocking)
...
if [ "$METHOD" == "auto" ]; then
    ...
    if command -v pre-commit &> /dev/null; then
        ...
        METHOD="native"
        echo -e "${YELLOW}ℹ Pre-commit not found, using native hooks${NC}"
    fi
    echo ""
fi
...
echo -e "${GREEN}✓ Manifest integrity verified${NC}"
echo ""
...
if [ -n "$MODIFIED_TEMPLATES" ]; then
    ...
    if ! git diff --cached --name-only | grep -q '.claude-kit-manifest.json'; then
        echo ""
        echo -e "${YELLOW}⚠️  WARNING: Template modified but manifest not staged${NC}"
        echo ""
        ...
        echo "  3. Stage: git add template/.claude-kit-manifest.json"
        echo ""
        echo -e "${YELLOW}ℹ This is a warning only - commit will proceed${NC}"
        echo ""
    else
    ...
    echo -e "${GREEN}✓ No version check needed${NC}"
    echo ""
fi
...
YAML_EOF
        ...
        echo -e "${YELLOW}ℹ .pre-commit-config.yaml already exists${NC}"
    fi
...
echo "  ✓ Manifest integrity (hash verification)"
echo "  ✓ Version updates (warning only)"
echo ""
