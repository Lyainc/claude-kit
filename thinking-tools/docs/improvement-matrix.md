...
| W3 | 상태 휘발성 | STATE 블록이 컨텍스트 compaction 시 손실 위험, 스킬마다 포맷 상이 | 전 스킬 | resolved | P1 | PR #179, #432 | `reference/state-contract.md`가 compaction 복원 규칙과 `<!-- STATE:CHECKPOINT -->` 마커를 단일화했고, adversarial-review·unknown-discovery·expert-panel이 이 파일을 인용해요. 잔여 중복(unknown-discovery/build-spec의 INTERVIEW_STATE.md 저장/복원 규칙 사본)은 #432에서 `reference/interview-state.md`로 합류시켜 해소했어요 |
