...
> vault가 아닌 목적지(`repo_path`·`stdout`·`local_ephemeral`·`github`)는 CON-1 게이트 대상이 아니에요. 특히 `/handoff`는 vault를 **안 건드리고** 로컬 gitignored `resume.md` 또는 stdout만 산출하므로 `gated`가 아니라 `success`예요(§2 #4 주의).
...
| 4 | `handoff` | `handoff` | **`/handoff`** (vault-bridge 커맨드) | vault-bridge 커맨드 (로컬 핸드오프 · **vault 비경유** → ③ "vault 운반"에 미해당) | user-initiated 슬래시 커맨드 (`disable-model-invocation`) | `local_ephemeral` (`.claude-kit/vault-bridge/resume.md`, gitignored) 또는 `stdout` | `success` (**vault 미사용 → CON-1 비대상**) |
...
이 문서 = **논리 계약**(어댑터 호출 규약·매핑·체이닝). ②를 단일 플러그인으로 묶을지(graphify=user-level 스킬·OVM note·spec-first 등 출력 자산이 이미 도메인별 분산 — 물리 위치 정정은 `output-layer-structure-adr.md` §0 참조) 분산 유지할지의 **물리 구조는 #102(G3 wave)** 결정이에요. 어댑터 계약은 구조 독립적이라 #102 어느 결과에도 불변 — #101 Acceptance는 #102 없이 충족(G2 쟁점 표 "출력 레이어 물리 구조" 행 정합). **#102 결정(2026-06-04): 분산(논리 계약) — `output-layer-structure-adr.md`.**
...
| **포맷×동작 매트릭스** (html=graphify, note=OVM note, goal-doc=spec-first, handoff=/handoff, session=/save-session, md저작=doc-concretize, md편집=doc-polish, issue=gh CLI) | §2(8매핑 표) |
