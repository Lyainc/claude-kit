...
# Phase-1 stamp + Phase-2 emit helper). Asserts: opt-in gate, schema-shaped emit
# line with the two retro counters, numeric duration when a start_ms is
...
( unset CLAUDE_KIT_TELEMETRY; "$SCRIPT" emit 999999 1 1 )
...
echo "$LINE" | jq -e '.meta.retro_items_processed==5 and .meta.items_deduped==2' >/dev/null || fail "meta counters wrong"
...
"$SCRIPT" emit "$START_A" 3 1
"$SCRIPT" emit "$START_B" 7 2
...
# 6. #528 batching regression guard: the 3 independent Phase-1 collect commands
# (stamp / report.py / sequence.py) must live in ONE fenced code block in
# retro/SKILL.md, not three separate ones (else the wrap chain pays 3
# Bash-tool turns instead of 1).
...
# 7. #580: Phase 2's emit call must pass START_MS as its first argument, not
# just the two counters (a partial edit that updated the helper but left
...
grep -q 'retro-telemetry.sh" emit "\$START_MS" "\$PROCESSED" "\$DEDUPED"' "$SKILL" \
  || fail "#580: Phase 2 emit call site does not pass \$START_MS first"
...
OUT="$(CLAUDE_KIT_TELEMETRY_DIR="$D617" "$SCRIPT" emit 999999 1 0 2>"$TMP/stderr-617")"
RC=$?
chmod 700 "$D617"
