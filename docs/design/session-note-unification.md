...
> **⚠️ SUPERSEDED (2026-07-08) — 이 문서는 역사 기록이에요.** 여기서 설계한 `wrapup`/`vault-daily` 스킬, `vault-searcher` Mode-4의 record/handoff/quick 분기, Stop-hook 자동 제안은 전혀 이 형태로 지어지지 않았어요. 한동안은 session 기록이 `/save-session` 슬래시 커맨드 하나(`vault-bridge/commands/save-session.md`, `session` format의 출력 어댑터, `docs/design/output-adapter-contract.md` 정합)로 일원화됐었는데, 2026-07-08 `docs/specs/save-session-ore-repurpose.yaml`(D1)로 session-note 저작 자체가 폐기됐어요 — `/save-session`은 이제 `type:capture` 원석을 `inbox/`에 적재하는 캡처 문이에요. 이 문서가 상상하지 못한 인수인계/continuation 기능은 이제 claude-kit 바깥의 머신 레벨 `session-close` 스킬이 가져갔어요(G26). 아래 본문은 그 결정에 이르기까지의 설계 사고 기록으로만 읽으세요 — 새로 구현할 근거로 쓰지 마세요.
