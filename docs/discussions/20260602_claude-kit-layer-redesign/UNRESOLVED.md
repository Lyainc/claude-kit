...
> **RESOLVED (2026-06-04, #102)**: (U-2 분산 결정의 하류 — 아래 **U-2 RESOLVED** 먼저 참조) 행선지 = **in-place reframe**(doc-concretize/doc-polish는 thinking-tools 잔류, 역할만 재정의). 분산=논리 계약이라 물리 이동 불필요 — `docs/design/output-layer-structure-adr.md` §2.5. 방향은 #102가 지정, 물리 실행은 #103.
