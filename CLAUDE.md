...
- **obsidian-vault-manager** (`obsidian-vault-manager/`): Obsidian vault 지식 관리 — 에이전트 2개 (vault-knowledge-manager, vault-file-organizer) + 스킬 2개 (audit, base — 입구 `/capture`·`/note`는 #480으로 retire, vault-bridge `/vault-save`가 승계; `/wiki`는 #645로 배포 단위가 vault-bridge로 이관) + reference docs (`reference/vault-audit-rules.md`, `reference/obsidian-bases-schema.md` 등) + shell primitives (`scripts/ovm-primitives.sh`). vault에 남는 OVM의 몫은 감사(`/audit`)·뷰(`/base`)로, 두 에이전트는 초안·계획만 돌려주고 콘텐츠를 직접 쓰지 않습니다.
- **vault-bridge** (`vault-bridge/`): Obsidian vault I/O 브릿지 플러그인 — 에이전트 1개 (vault-searcher, haiku) + 훅 2종 (SessionStart / PreToolUse Write|Edit|Bash) + 스킬 5개 (`/vault-save`, `/vault-link`, `/vault-manifest-refresh`, `/vault-commit`, `/wiki`) + Python scripts (`generate-manifest.py`, `vault-commit-message.py`, `manifest-wiki-match.py`). vault 검색 + git 커밋. vault 참고자료 입구(`/vault-save`)도 컴파일(`/wiki`)도 이 플러그인이 배포합니다. wiki = v5 A-layer LLM wiki 컴파일(`vault/wiki/`, AI recall 主, provenance 추적, 게이트된 명시 액션). **배포단위≠레이어(#645)**: `/wiki`는 여전히 ④ 지식베이스 작업이고 #304 straddler 판정도 그대로예요 — 어느 플러그인이 배포하느냐만 ③ vault-bridge로 바뀐 겁니다 (feedback-loop과 같은 구분).
...
vault-bridge registers 2 hook handlers (SessionStart manifest refresh, PreToolUse Write|Edit|Bash write-role-contract enforcement) + 5 skills (`/vault-save`, `/vault-link`, `/vault-manifest-refresh`, `/vault-commit`, `/wiki`). All hooks are deterministic shell scripts — no per-turn LLM cost. Full hook/skill detail + the Write Role Contract (vault reads are haiku-delegable, writes are not): [docs/REFERENCE.md](docs/REFERENCE.md#vault-bridge-hooks--skills).
...
| Session record | N/A (#645로 `wiki` 배포 단위 이관 — 레이어 판정은 그대로 ④) | `wiki` (compiled session knowledge → `wiki/`) — `/save-session` retired #331, wiki-first; `/vault-save` (raw session ore → `sources/`); `/vault-commit` commits the vault |
