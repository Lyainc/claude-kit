...
|---|---|---|---|---|
| next-goal | 세션 후보·현재 상태·재개 지점·관련 파일·보호 조건, 실용적 가치와 관련성, 최소 크기 없음 | NEXT/POOL/RUNNERS + 관찰 가능한 GOAL; 무가치 후보는 GOAL 없음; 읽기 전용 | 직접 호출은 `/goal` 4,000자 이내, caller는 결과를 한 번 배치 | 일반 GOAL, 호출 API를 흉내 내지 않고 설치된 계약을 직접 적용 |
