...
| 쟁점 | 선택지 | 권장 | 근거 |
|------|--------|------|------|
| `/save-session` 저작 레시피 위치 | A) `session-note-recipe.md` 보존 (현행 인라인 참조) / B) G2 출력 레이어로 완전 이전 후 참조 | B (G2 완료 후) | G2가 저작 흡수하면 recipe는 ② 소유. G4는 진입점 명세만 유지 |
...
| G4 착수 시점 | G2 fully shipped 후 vs G2 partial(저작 흡수 일부) 후 | G2 fully shipped 후 | G2가 저작을 흡수하기 전에 G4가 recipe를 이전하면 dangling reference 발생. 게이트 조건 = G2 Done |
| `session-note-recipe.md` 잔존 여부 | G2 이전 후 삭제 vs vault-bridge 내 보존(참조용) | G2 이전 후 삭제 또는 redirect stub 남기기 | 중복 유지는 드리프트 위험. stub + pointer면 기존 링크 보존 |

**게이트 조건**: G2(출력 레이어·저작 흡수)가 `status: done`으로 전환돼야 G4 착수 가능.
...
2. **S2 슬래시 명령 저작 책임 연결** → 바인딩: `executor` (sonnet) | 대상 파일: `vault-bridge/commands/save-session.md`, `vault-bridge/commands/handoff.md`, `vault-bridge/commands/save-plan-doc.md` | 산출: 각 명령 파일이 G2의 출력 레이어 레시피를 참조하도록 Step 1 로드 경로 업데이트; 저작 단계 = 메인컨텍스트 전용 명시; 불필요해진 인라인 레시피 내용 제거 또는 redirect | 검증: 각 명령 파일에 vault-searcher 위임 코드가 없음을 grep으로 확인

3. **S3 session-note-recipe.md 정리** → 바인딩: `executor` (sonnet) | 대상 파일: `vault-bridge/reference/session-note-recipe.md` (현행 위치 확인 필요) | 산출: G2로 이전된 레시피 내용 삭제 + redirect stub 또는 파일 제거; dangling reference 없음 확인 | 검증: grep -r "session-note-recipe" vault-bridge/ 로 참조 일관성 확인
...
- **게이트**: G2(출력 레이어 저작 흡수)가 `status: done`이어야 G4 착수 가능. G2가 session-note 레시피를 흡수하기 전에 S2·S3 진행하면 dangling reference 발생해요.
- **wave=3 병렬성**: G4는 G3(thinking-tools 강화)와 다른 플러그인 영역이라 wave 3에서 병렬 진행 가능. G3 완료를 기다릴 필요 없어요.
- **G2 의존 범위**: S1(비대칭 명문화)은 G2와 무관하게 선행 가능. S2·S3(레시피 이전·정리)만 G2 Done 게이트 적용.
...
- **크로스청크 게이트**: G2가 먼저 merged돼야 G4의 PR을 base=main으로 생성할 수 있어요. chained PR 시 CLAUDE.md PR Workflow 섹션의 rebase 절차 적용해요.
